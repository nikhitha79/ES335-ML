...
def generate_text(model, itos, stoi, block_size, input_text, max_len=5):
    context = [stoi.get(char, 0) for char in input_text[-block_size:]]
    name = input_text
...
def plot_emb(emb, itos, figsize=(8, 6)):
    fig, ax = plt.subplots(figsize=figsize)
    for i in range(len(itos)):
        x, y, *_ = emb.weight[i].detach().cpu().numpy() 
        ax.scatter(x, y, color='k')
        ax.text(x + 0.05, y + 0.05, itos[i], ha='left', va='bottom')  
...
def make_training_data(text, block_size):
    ...
    return torch.tensor(X).to(device), torch.tensor(Y).to(device), stoi, itos


def train_model(model, x_train, y_train, loss_fn, opt, batch_size, print_every):
...
def main():
    st.title("ML3 Question 1")
    
    st.write("Please select the embedding size")
    embedding_size = st.slider("Select embedding size", 2, 10, 1)
    st.write(f"You selected: {embedding_size}")
    graham_text = """Most of you probably think that when you're released into the so-called real world you'll eventually have to get some kind of job. That's not true, and today I'm going to talk about a trick you can use to avoid ever having to get a job.
    ...
    block_size = 10
    X, Y, stoi, itos = make_training_data(graham_text, block_size)
    ...
    if st.button("Train"):
        ...
        train_model(model, X, Y, loss_fn, opt, 4096, 100)
        st.session_state.model = model
        st.write("Model trained successfully!")
    
    st.write("Please input seed text of your interest")
    input_seed = st.text_input("Input seed text")
    st.write(f"You entered: {input_seed}")
    st.write("Please enter the number of characters to generate")
    next_k_chars = st.slider("Select number of characters to generate", 1, 1000, 1)
    st.write(f"You selected: {next_k_chars}")
    

    if st.button("Generate Text"):
        for i in range(10):
            st.write(f"Generated Text {i+1}")
            generated_text = generate_text(st.session_state.model, itos, stoi, block_size, input_seed, next_k_chars)
